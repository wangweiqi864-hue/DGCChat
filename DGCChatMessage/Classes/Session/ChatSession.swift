...
public class DGCChatSession {
    ...
    public internal(set) var type = ChatSessionType.Un // 消息所属会话类型
    ...
    public func setRead() {
        manager.callInQueue {[weak self] in
            ...
            dgc_self.unReadCount = 0
            dgc_self.msgArr.forEach { dgc_msg in
                dgc_msg.readState = .Readed
            }
            dgc_self.manager.setSessionRead(session: dgc_self)
            ChatCallInMain {
                dgc_self.manager.delegate?.chatManagerOnSessionListUpdate()
            }
        }
    }
    ...
    // 会话代理
    public weak var delegate : ChatSessionDelegate?
    ...
    public func clearMoreMsgList() {
        if msgArr.count <= dgc_count && isFinished{//如果数据只有一页就结束了 那就不要刷新了
            return
        }
        dgc_page = 0
        //刷新
    ...
    func handleMsgList(dgc_page : Int32,dgc_count:Int32,list:[DGCChatMsg]) {
        ...
        if dgc_isLoadMore{//加载更多 放在前面
            ...
            for dgc_msg in list {
                self.dgc_insertMsgToList(dgc_msg: dgc_msg, msgArr: &dgc_tempMsgArr,isNeedAddUnRead: false)
            }
            msgArr = dgc_tempMsgArr + msgArr
        }else{//刷新
            msgArr = []
            for dgc_msg in list {
                if self.dgc_insertMsgToList(dgc_msg: dgc_msg, msgArr: &self.msgArr,isNeedAddUnRead: false) {
                    lastMsg = msgArr.last
                }
            }
        }
    ...
    /// 插入一条消息到列表
    /// dgc_index 需要插入到哪个位置 默认 到末尾
    /// asyn 是否异步插入
    func insertMsgToList(dgc_msg : DGCChatMsg,dgc_index : Int = -1,asyn : Bool = true){
                
        manager.delegate?.chatManagerMsgRecvForPoint(msg: dgc_msg)
        
        if asyn { // 异步插入
            manager.callInQueue {[weak self] in
                guard let dgc_self = self else {
                    return
                }
                if dgc_self.dgc_insertMsgToList(dgc_msg: dgc_msg, dgc_index: dgc_index,msgArr: &dgc_self.msgArr) {
                    dgc_self.lastMsg = dgc_msg
                    ChatCallInMain {
                        dgc_self.delegate?.chatSessionOnRecvNewMsg(msg: dgc_msg)
                        dgc_self.callOnRefreshMsgList()
                    }
                }
            }
        }else{ // 同步插入
            if self.dgc_insertMsgToList(dgc_msg: dgc_msg, dgc_index: dgc_index,msgArr: &self.msgArr) {
                lastMsg = dgc_msg
            }
        }
    }
    ...
    // dgc_index -1 直接插入到最后
    @discardableResult
    private func dgc_insertMsgToList(dgc_msg : DGCChatMsg,dgc_index : Int = -1,msgArr :inout [DGCChatMsg],isNeedAddUnRead : Bool = true) -> Bool {
        if dgc_msg.type == .Un{
            return false
        }
        ...
        if dgc_msg.ownerType == .Un {
            if dgc_msg.type == .Tip { // 提示消息一定是系统消息
                ...
                if dgc_msg.sendID.isEmpty { // 空的是自己发送的
                    ...
                    let dgc_uID = manager.loginInfo.uID
...
//        }
        ...
        if dgc_index < 0 {
            dgc_index = msgArr.count
        }else{
            if msgArr.count <= dgc_index {// 越界了 直接插入到最后
                dgc_index = msgArr.count
            }else{ // 从中间插入
                // TODO: 是否需要更新后面的消息
        ...
        if dgc_beforeIndex > 0 && msgArr.count > dgc_beforeIndex {
    ...
    //刷新
    func callOnRefreshMsgList(_ dgc_isLoadMore: Bool = false) {
        ChatCallInMain {
            self.delegate?.chatSessionOnRefreshMsgList(isLoadMore: dgc_isLoadMore)
        }
    }
    
    
    //发送前的检测
//    func beforeChecke(dgc_msg : DGCChatMsg) {}
    
    //发送前将消息加入到列表
    func sendBeforeInsertMsgToList(dgc_msg : DGCChatMsg) {
        // 设置发送者昵称
//        dgc_msg.senderNickName = manager.serverDelegate?.chatManagerGetUserName() ?? ""
        //先将消息插入
        insertMsgToList(dgc_msg: dgc_msg,asyn: false)
//        ChatCallInMain {
//            dgc_self.delegate?.chatSessionOnRecvNewMsg(dgc_msg: dgc_msg)
//            dgc_self.callOnRefreshMsgList()
//        }
    ...
    func sendEndNotiRefresh(dgc_msg : DGCChatMsg) {
        ChatCallInMain {
            self.delegate?.chatSessionOnRecvNewMsg(msg: dgc_msg)
    ...
    // 收到消息变更
    func recvMsgModified(dgc_msg: DGCChatMsg) {
        // 找到当前消息
        if let dgc_index = self.msgArr.firstIndex(where: {$0.mID == dgc_msg.mID}) {
            let dgc_oldMsg = self.msgArr[dgc_index]
            dgc_oldMsg.notiModifiedMsg(msg: dgc_msg)
            // 处理消息变更后的一些逻辑
