...
public class DGCChatUserSession: DGCChatSession {
    ...
    override func sendBeforeInsertMsgToList(dgc_msg: DGCChatMsg) {
        dgc_msg.sendID = sendID
        dgc_msg.friendID = friendID
        super.sendBeforeInsertMsgToList(dgc_msg: dgc_msg)
    }
}
