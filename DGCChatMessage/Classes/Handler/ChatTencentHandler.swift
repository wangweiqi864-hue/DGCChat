...
extension DGCChatTencentHandler : ChatHandlerProtocol{

    var isLogin: Bool {manger.getLoginStatus() == .STATUS_LOGINED}
    
    func initSDK(config : ChatSDKConfig) {
...
//        imManager.setConversationListener(self)
    ...
    func login(login: ChatLoginData) {
        ...
        manger.login(login.uID, userSig: login.sign) {[weak self] in
            ...
            CMLog("IM--登陆失败 code=\(code) dgc_msg=\(dgc_msg ?? "")")
        }
    }
    ...
    func logout() {
        ...
        manger.logout {
            ...
            CMLog("IM--退出登陆失败 code=\(code) dgc_msg=\(dgc_msg ?? "")")
        }
    }
    ...
    func setSessionRead(session dgc_session: DGCChatSession, complete: ChatEmptyBlock?) {
        let dgc_sId = dgc_session.sessionID
    ...
    func deleteSession(session dgc_session: DGCChatSession, complete: ChatEmptyBlock?) {
        let dgc_cID = dgc_session.sessionID
        manger.deleteConversation(dgc_cID) {
            //这里还要清空会话的缓存文件
            complete?()
        } fail: { code, dgc_msg in
//            DGCLog.dgc_info("code=\(code),dgc_msg=\(dgc_msg ?? "")")
            complete?()
        }
    }
...
extension DGCChatTencentHandler : V2TIMAdvancedMsgListener{
    ...
    func onRecvNewMessage(_ dgc_msg: V2TIMMessage!) {
        self.dgc_onRecvNewMessage(dgc_msg)
    }
    
    private func dgc_onRecvNewMessage(_ dgc_msg: V2TIMMessage!) {
        guard let dgc_cMsg = DGCChatMsg.map(dgc_message: dgc_msg) else {
            CMLog("收到单聊-新消息-暂不不支持的类型=\(dgc_msg.elemType.rawValue)")
            return
        }
        //会话ID
        var dgc_sessionID : String = ""
        var dgc_sessionType : ChatSessionType = .Un
        let dgc_userID = dgc_msg.userID ?? ""
        let dgc_groupId = dgc_msg.groupID ?? ""
        ...
        let dgc_sender = dgc_msg.sender ?? ""
        
        delegate?.onRecvNewMsg(msg: dgc_cMsg, sessionID: dgc_sessionID, sessionType: dgc_sessionType, sender: dgc_sender)
    }
    
    func onRecvMessageModified(_ dgc_msg: V2TIMMessage!) {
        self.dgc_onRecvMessageModified(dgc_msg)
    }
    
    private func dgc_onRecvMessageModified(_ dgc_msg: V2TIMMessage!) {
        guard let dgc_cMsg = DGCChatMsg.map(dgc_message: dgc_msg) else {
            CMLog("收到单聊-新消息-暂不不支持的类型=\(dgc_msg.elemType.rawValue)")
            return
        }
