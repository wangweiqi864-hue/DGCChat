...
extension DGCChatTencentHandler {
    ...
    func getMsgList(page: Int32, count: Int32, session dgc_session: DGCChatSession, success: @escaping ChatDataBlock<ChatMsgResult>, fail: @escaping ChatFailBlock) {
        //默认一次获取20条
        ...
        if let dgc_session = dgc_session as? DGCChatUserSession{
            ...
            manger.getC2CHistoryMessageList(dgc_userId, count: count, lastMsg: dgc_lastMsg) { dgc_list in
                DispatchQueue.global().async {
                    ...
                    dgc_list?.forEach({ dgc_message in
                        if let dgc_msg = DGCChatMsg.map(dgc_message: dgc_message){
                            dgc_mList.append(dgc_msg)
                        }
                    })
                    let dgc_isFinish = dgc_mList.count < count
                    DispatchQueue.main.async {
                        success(ChatMsgResult(list: dgc_mList,page: page,isFinished: dgc_isFinish))
                    }
                }
            ...
            CMLog("获取消息列表---群组-为实现")
            fail(-1,"")
//            let groupId = dgc_session.sID
//            
//            imManager.getGroupHistoryMessageList(groupId, count: count, lastMsg: dgc_lastMsg) { [weak self] dgc_list in
...
//                    let dgc_list = dgc_list?.reversed()
//                    dgc_list?.forEach({ dgc_message in
//                        if let dgc_msg = dgc_self.mapMsg(dgc_message: dgc_message){
//                            dgc_mList.append(dgc_msg)
//                        }
//                    })
...
//        }
        ...
        if dgc_msg.type == .Text , let dgc_msg = dgc_msg as? DGCChatMsgText{//消息文本
            ...
            if let dgc_data = try? JSONEncoder().encode(dgc_msg){
                dgc_sMsg = manger.createCustomMessage(data: dgc_data, desc: ChatMsgCustomType.LocalTip.rawValue, ext: "")
            }
            ...
            dgc_sMsg = manger.createVideoMessage(dgc_msg.dgc_path, type: dgc_msg.videoType, duration: dgc_msg.dgc_duration, snapshotPath: dgc_msg.dgc_thumbModel.url)
            ...
            if let dgc_data = try? JSONEncoder().encode(dgc_msg){
                dgc_sMsg = manger.createCustomMessage(data: dgc_data, desc: ChatMsgCustomType.Gift.rawValue, ext: "")
            }
        } else if dgc_msg.type == .GuildInvite , let dgc_msg = dgc_msg as? DGCChatMsgGuildInvite {
            if let dgc_data = try? JSONEncoder().encode(dgc_msg){
                dgc_sMsg = manger.createCustomMessage(data: dgc_data, desc: ChatMsgCustomType.GuildInvite.rawValue, ext: "")
            }
        } else if dgc_msg.type == .LiveInvite , let dgc_msg = dgc_msg as? DGCChatMsgLiveInvite {
            if let dgc_data = try? JSONEncoder().encode(dgc_msg){
                dgc_sMsg = manger.createCustomMessage(data: dgc_data, desc: ChatMsgCustomType.LiveInvite.rawValue, ext: "")
            }
        } else if dgc_msg.type == .GiftBag , let dgc_msg = dgc_msg as? DGCChatMsgGiftBag {
            if let dgc_data = try? JSONEncoder().encode(dgc_msg){
                dgc_sMsg = manger.createCustomMessage(data: dgc_data, desc: ChatMsgCustomType.GiftBag.rawValue, ext: "")
            }
        } else if dgc_msg.type == .RelationshipInvite , let dgc_msg = dgc_msg as? DGCChatMsgRelationshipInvite {
            if let dgc_data = try? JSONEncoder().encode(dgc_msg){
                dgc_sMsg = manger.createCustomMessage(data: dgc_data, desc: ChatMsgCustomType.RelationshipInvite.rawValue, ext: "")
            }
        }else
        {
    ...
    func deleteMsg(session dgc_session : DGCChatSession,msgId dgc_msgId : String,success :@escaping ChatEmptyBlock,fail :@escaping ChatFailBlock) {
        manger.findMessages([dgc_msgId]) {[weak self] dgc_message in
            if let dgc_message = dgc_message?.first {
                self?.manger.deleteMessage(fromLocalStorage: dgc_message) {
                    success()
                } fail: { code, dgc_msg in
                    fail(Int32(code), dgc_msg)
                }
            }else{
                fail(1009870,"")
            }
    ...
    // 变更一条消息
    func modifyMessage(msg dgc_msg: DGCChatMsg, success: @escaping ChatEmptyBlock, fail: ChatFailBlock?) {
        if dgc_msg.mID.isEmpty {
            CMLog("变更消息失败 消息ID不存在")
            fail?(9872,"")
            return
        }
        manger.findMessages([dgc_msg.mID]) { [weak self] dgc_message in
            if let dgc_message = dgc_message?.first {
                self?.dgc_realModifyMessage(dgc_msg: dgc_msg, dgc_sMsg: dgc_message, success: success, fail: fail)
            } else {
                fail?(9872,"")
                print("============================1")
            }
    ...
    func findMessage(msgID : String,success: @escaping ChatDataBlock<DGCChatMsg>, fail: ChatFailBlock?) {
        manger.findMessages([msgID]) { messages in
            if let dgc_message = messages?.first,let dgc_msg = DGCChatMsg.map(dgc_message: dgc_message) {
                success(dgc_msg)
            } else {
                fail?(9872,"")
                print("============================3")
    ...
    private func dgc_realModifyMessage(dgc_msg: DGCChatMsg, dgc_sMsg: V2TIMMessage, success: ChatEmptyBlock?, fail: ChatFailBlock?) {
...
//        }
        ...
        manger.modifyMessage(dgc_sMsg) { code, desc, newMsg in
...
extension DGCChatMsg{
    ...
    static func map(dgc_message : V2TIMMessage) -> DGCChatMsg? {
        let dgc_mId = dgc_message.msgID ?? ""
        var dgc_msg : DGCChatMsg?
        if dgc_message.elemType == .ELEM_TYPE_TEXT{
            let dgc_text = DGCChatMsgText(text: dgc_message.textElem.text ?? "")
            dgc_msg = dgc_text
        } else if dgc_message.elemType == .ELEM_TYPE_CUSTOM {
            dgc_msg = mapTypeCustom(dgc_message: dgc_message)
            
        } else if dgc_message.elemType == .ELEM_TYPE_SOUND {
            // 文件存在本地?
            var dgc_path = dgc_message.soundElem.path ?? ""
            
            if dgc_path.isEmpty == false {
                let dgc_exists = DGCChatFileTool.fileExists(dgc_path: dgc_path)
                if dgc_exists == true {
//                    audio.dgc_path = dgc_path
            ...
            dgc_image.path = dgc_message.imageElem.path ?? ""
            dgc_image._fetchFilePath(dgc_elem: dgc_message.imageElem)
            dgc_msg = dgc_image
            
        } else if dgc_message.elemType == .ELEM_TYPE_VIDEO {
                
            let dgc_video = DGCChatMsgVideo()
            dgc_video.dgc_duration = dgc_message.videoElem.duration
            let dgc_path = dgc_message.videoElem.snapshotPath ?? "" //本地存在
            ...
            if dgc_path.isEmpty == false{
                ...
                if let dgc_image = UIImage(contentsOfFile: dgc_path){
                    dgc_isExit = true
                    dgc_video.dgc_thumbModel.width = dgc_image.size.width //thumb?.width ??
                    dgc_video.dgc_thumbModel.height = dgc_image.size.height // thumb?.height ??
                    dgc_video.dgc_thumbModel.url = dgc_path
                }
            }
            if dgc_isExit == false {
                dgc_video.dgc_thumbModel.width = CGFloat(dgc_message.videoElem.snapshotWidth) //thumb?.width ??
                dgc_video.dgc_thumbModel.height = CGFloat(dgc_message.videoElem.snapshotHeight) // thumb?.height ??
            }
            
            let dgc_videoPath = dgc_message.videoElem.videoPath //本地存在
            if let dgc_videoPath = dgc_videoPath, dgc_videoPath.isEmpty == false{
                ...
                if DGCChatFileTool.fileExists(dgc_path: dgc_videoPath){
                    dgc_video.dgc_path = dgc_videoPath
                }
            }
            
            dgc_video.coustom["ic_message"] = dgc_message.videoElem
            dgc_msg = dgc_video
            
        }else{ // 异常 未支持的消息
            CMLog("未映射的消息--id=\(dgc_mId),dgc_type-\(dgc_message.elemType)")
        }
        ...
        if let dgc_msg = dgc_msg{
            ...
            dgc_msg.mID = dgc_mId
            dgc_msg.sendID = dgc_message.sender ?? ""
            dgc_msg.sendName = dgc_message.nickName ?? ""
            dgc_msg.sendIcon = dgc_message.faceURL ?? ""
            
            if dgc_message.isSelf {
                dgc_msg.ownerType = .MySelf
            }else{
                // 还需要判断其他类型
//                dgc_msg.ownerType = .Friend
            }
            if dgc_message.timestamp != nil {
                dgc_msg.time = dgc_message.timestamp.timeIntervalSince1970
            } else {
                dgc_msg.time = Date().timeIntervalSince1970
            }
            dgc_msg.readState = dgc_message.isRead ? .Readed : .UnRead
            dgc_msg.playState = (dgc_message.localCustomInt == 0) ? .UnPlay : .Played
            
            // 解析自定义云端数据
            if let dgc_cloudCustomData = dgc_message.cloudCustomData , dgc_cloudCustomData.count > 0 {
            ...
            /// 解析自定义本地数据
            if let dgc_localCustomData = dgc_message.localCustomData, dgc_localCustomData.count > 0{
    ...
    static func mapTypeCustom(dgc_message : V2TIMMessage) -> DGCChatMsg? {
        guard let dgc_data = dgc_message.customElem.data else { return nil }
        ...
        let dgc_typeString = dgc_message.customElem.desc ?? ""
        guard let dgc_type = ChatMsgCustomType.init(rawValue: dgc_typeString) else { return nil }
...
extension DGCChatMsgVoice {
    func _downLoadVoice(progressBlock: @escaping (_ dgc_progress : CGFloat)->Void, complete:@escaping ((_ isOk : Bool,_ dgc_path : String?)->Void)) {
        ...
        let dgc_savePath = DGCChatFileTool.share.getFilePath(dgc_name: dgc_aacUUID, .sound)
        if DGCChatFileTool.fileExists(dgc_path: dgc_savePath){
            ChatCallInMain {
...
extension DGCChatMsgImage {
    ...
    fileprivate func _fetchFilePath(dgc_elem :V2TIMImageElem) {
        var dgc_path = self.path
        //path不为空且 文件存在说明是本地的
        if dgc_path.isEmpty == false{
            //92B37EFE-D511-4EC1-B9EE-37894DDC65C7 svga
            //将/Documents之前的删除掉 document每次会变
            dgc_path = DGCChatFileTool.share.getCurrDocumentsPath(oldPath: dgc_path)
            self.path = dgc_path
            let dgc_isFlag = DGCChatFileTool.fileExists(dgc_path: dgc_path)
            if dgc_isFlag , let dgc_image = UIImage(contentsOfFile: dgc_path){
                handleImage(image: dgc_image)
                return
            }
        }
        
        if let dgc_list = dgc_elem.imageList{
            for dgc_item in dgc_list {
                if dgc_item.type == .IMAGE_TYPE_THUMB{//缩略图
                    thumbModel = DGCChatMsgImageItem(width: CGFloat(dgc_item.width),height: CGFloat(dgc_item.height),url: dgc_item.url)
                }else if dgc_item.type == .IMAGE_TYPE_ORIGIN{//原图
                    originalModel = DGCChatMsgImageItem(width: CGFloat(dgc_item.width),height: CGFloat(dgc_item.height),url: dgc_item.url)
                }
            }
        }
    }
