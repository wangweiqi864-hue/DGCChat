...
extension DGCChatTencentHandler {
    ...
    func getSessionList(page: Int32, count: Int32, success: @escaping ChatDataBlock<ChatSessionResult>, fail: @escaping ChatFailBlock) {
        ...
        manger.getConversationList(dgc_currNextSeq, count: count) {[weak self] dgc_list, nextSeq, isFinished in
            if let dgc_self = self{
                DispatchQueue.global().async {
                    dgc_self.nextSeq = nextSeq
                    let dgc_list = dgc_list?.map({DGCChatSession.map($0)}) ?? []
                    success(ChatSessionResult(list: dgc_list,page: page,isFinished: isFinished))
                }
            }
        } fail: { code, msg in
...
extension DGCChatSession{
    ...
    static func map(_ conversation : V2TIMConversation) -> DGCChatSession {
        ...
        if let dgc_lastMsg = conversation.lastMessage{//存在最后一条消息
            ...
            dgc_session.lastMsg = DGCChatMsg.map(dgc_message: dgc_lastMsg) // 可能会导致 最后一条消息没有 会话id
        }
        return dgc_session
