...
public class DGCChatMsg {
    ...
    public internal(set) var sessionType = ChatSessionType.Un // 消息所属会话类型
    
    public internal(set) var type = ChatMsgType.Un // 消息类型
    
    public internal(set) var ownerType = ChatMsgOwnerType.Un // 消息发送类型
    
    public internal(set) var sendState = ChatMsgSendState.OK // 消息发送状态
    
    public internal(set) var readState = ChatMsgReadState.UnRead // 消息读取状态
    
    public internal(set) var isLocal : Bool = false // 是否是本地存储的消息
    
    public internal(set) var playState = ChatMsgPlayState.UnPlay // 消息 播放状态 主要用于音频和礼物
    
    // 发送进度 发送文件时才有
    public internal(set) var progress : CGFloat = 0{
        didSet{
            if let dgc_sendProgressBlock = dgc_sendProgressBlock {
                ChatCallInMain {
                    dgc_sendProgressBlock(self.progress)
                }
            }
        }
    }
    //发送进度回调
    public var dgc_sendProgressBlock : ChatDataBlock<CGFloat>?
...
public enum ChatCloudCustomDataType: Int, Codable {
    case CCDT_NIL = 0
...
public enum ChatSystemCloudCustomDataType: Int, Codable {
    case SCCDT_NIL = 0
...
public enum RelationshipStatus : Int32, Codable{
    case un
    case accept = 1 // 同意
    case reject = 4 // 拒绝
...
public class DGCChatMsgCloudCustomData : Codable{
    ...
    public var custom_type: ChatCloudCustomDataType?
    /// 内容 没有则传空
...
//    public var payment_info : IMChatPaymentInfo?
    ...
    // 服务端标记的消息类型
    public var sys_custom_type: ChatSystemCloudCustomDataType?
    // 透传msg_key
    public var msg_key : String?
    
    /// 关系邀请状态
    public var relationshipStatus = RelationshipStatus.un
