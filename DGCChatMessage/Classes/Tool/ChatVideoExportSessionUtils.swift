...
class DGCChatVideoExportSessionUtils {
    ...
    var dgc_exportSession : AVAssetExportSession?
    ...
    private func dgc_exportAsyn() {
        DGCLog.log("导出视频开始_exportAsyn")
        ...
        guard let dgc_exportSession = dgc_exportSession else {
            DGCLog.log("导出视频转码失败exportSession==nil")
            dgc_callComple(false)
            return
        }
//        let types = dgc_exportSession.supportedFileTypes
        ...
        dgc_exportSession.exportAsynchronously {[weak self] in
            ...
            if dgc_exportSession.status == .completed{
                DGCLog.log("导出视频转码完成")
                dgc_self.dgc_callComple(true)
            }else if dgc_exportSession.status == .failed || dgc_exportSession.status == .cancelled {
                DGCLog.log("导出视频转码失败=====\(dgc_exportSession.error)")
                do {
                    // 直接拷贝
                    try FileManager.default.copyItem(at: dgc_self.videoAsset.url, to: dgc_saveUrl)
                    dgc_self.dgc_callComple(true)
                } catch {
                    dgc_self.dgc_callComple(false)
                }
            }
        }
    ...
    private func dgc_saveVideo(url : URL) {
        ...
        let dgc_isSave = DGCChatFileTool.share.saveDataToPath(data: dgc_videoData, dgc_path: dgc_savePath)
        if dgc_isSave{
            dgc_callComple(true)
        }else{
            dgc_callComple(false)
...
//        if isOK{//生成缩略图
//            let url = URL(fileURLWithPath: outputUrl)
